...
from groups.api.serializers import (FellowshipSerializer, FellowshipMembershipSerializer, CellGroupMembershipSerializer,
                                    ChurchGroupMembershipSerializer,
                                    MinistryMembershipSerializer)
# TODO import each componet singly
from groups.models import (Fellowship, FellowshipMembership, CellGroup, CellGroupMembership, ChurchGroup,
                           ChurchGroupMembership, Ministry, MinistryMembership, )
...
class GetMembersOfFellowshipWithID_MatchPattern(APIView):
    ...
    def get(self, request, id, pattern):
        fellowship_membership = FellowshipMembership.objects.filter(fellowship_id=id,
                                                                    member__member__first_name__icontains=pattern)
        data = FellowshipMembershipSerializer(fellowship_membership, many=True).data
        return Response(data)
