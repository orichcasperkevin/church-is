...
class OfferingFromService(APIView):
    def get(self,request):
        dict = []
        for month in range(1,today.month + 1):
            total_from_services = Offering.objects.filter(service__isnull=False,date__month=month).aggregate(Sum('amount'))['amount__sum'] or 0
            total_from_services_last_month = Offering.objects.filter(service__isnull=False,date__month=month-1).aggregate(Sum('amount'))['amount__sum'] or 0
            ...
            for type in ServiceType.objects.all():
                total_from_type =  Offering.objects.filter(service_id=type.id,service__isnull=False,date__month=month).aggregate(Sum('amount'))['amount__sum'] or 0
                avg_from_type =  Offering.objects.filter(service_id=type.id,service__isnull=False,date__month=month).aggregate(Avg('amount'))['amount__avg'] or 0
                total_from_type_last_month =  Offering.objects.filter(service_id=type.id,service__isnull=False,date__month=month-1).aggregate(Sum('amount'))['amount__sum'] or 0
...
class OfferingByType(APIView):
    def get(self,request):
        dict = []
        for month in range(1,today.month + 1):
            total_offering = Offering.objects.filter(member__isnull=False,date__month=month).aggregate(Sum('amount'))['amount__sum'] or 0
            total_offering_last_month = Offering.objects.filter(member__isnull=False,date__month=month-1).aggregate(Sum('amount'))['amount__sum'] or 0
            ...
            for type in OfferingType.objects.all():
                total = Offering.objects.filter(member__isnull=False,date__month=month,type_id=type.id).aggregate(Sum('amount'))['amount__sum'] or 0
                total_last_month = Offering.objects.filter(member__isnull=False,date__month=month-1,type_id=type.id).aggregate(Sum('amount'))['amount__sum'] or 0
                avg_per_member = Offering.objects.filter(member__isnull=False,date__month=month,type_id=type.id).aggregate(Avg('amount'))['amount__avg'] or 0
